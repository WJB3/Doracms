    ...
    async removes() {
        ...
        try {
            ...
            if (!_.isEmpty(oldUser)) {
                if (oldUser.id === ctx.session.adminUserInfo.id ||
                    leftAdminUser == 1) {
    ...
    async getBasicSiteInfo() {
        ...
        try {
            ...
            for (let i = 0; i < fullResources.length; i++) {
                ...
                if (resourceObj.source_type == '1' && !_.isEmpty(ctx.session.adminUserInfo)) {
                    let adminPower = await ctx.helper.getAdminPower(ctx);
                    if (adminPower && adminPower.indexOf(resourceObj.id) > -1) {
            ...
            for (const pathItem of powerPathMaps) {
                ...
                if (this.app.config.env == 'local') {
                    ...
                    if (fs.existsSync(modulePkgPath)) {
                        ...
                        renderMap.push({
                            name: pathItem,
                            path: `${this.app.config.origin + '/cms/plugins' + this.app.config.static.prefix}/sadmin/${pathItem}/js/app.js`
                        })
                    }
