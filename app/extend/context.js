...
module.exports = {
    ...
    async getPageData() { 
        ...
        if (!_.isEmpty(pageData.site)) {
            ...
            if (ctx.pageType == 'index' || ctx.pageType == 'all') { // 首页
                let {
                    docs,
                    pageInfo
                ...
                if (payload.defaultUrl) {
                    ...
                    if (pageData.cateInfo.cate_type == '2') {
                        payload.cate_type = '2';
                    }
                ...
                pageData.posts = docs;
                pageData.pageInfo = pageInfo;
            } else if (ctx.pageType == 'tags') { // 标签所属
                let docs = await ctx.helper.reqJsonData('contentTag/getList', {
                    isPaging: '0'
                });
                pageData.tags = docs;
        ...
        if (targetTempPage.indexOf('users/') == '0') {
            ...
            console.log(defaultTemp.alias + '/' + targetTempPage)
            await ctx.render(defaultTemp.alias + '/' + targetTempPage, pageData);
        }
    },


    async getGoodsDetailPageData() {  
    ...
    async getGoodsCategoryPageData() {  
