 ...
 * @Last Modified time: 2020-11-07 21:27:43
 */
'use strict';
...
module.exports = {
        ...
        if (ctx.service.contentCategory) {
            try {
                ...
                if (!_.isEmpty(documtentCates) && action == 'get') {
                    ...
                    let cates = await ctx.service.contentCategory.find({
                        isPaging: '0'
                    }, {
    ...
    async initResourceData(ctx, pluginInfos = {}, type = 'install') {
        ...
        if (!_.isEmpty(pluginInfos)) {
            ...
            if (type == 'install') {
                ...
                let randomResource = await ctx.service.adminResource.item({
                    query: {
                        parentId: '0'
                    },
                    attributes: ['id']
                ...
                let targetResourceId = randomResource.id;
                ...
                let targetParentResource = await ctx.service.adminResource.item({
                    query: {
                        routePath: alias,
                        label: `${alias}Manage`
                    }
                ...
                if (!_.isEmpty(targetParentResource)) {
                    await ctx.service.adminResource.removes(targetParentResource.id, 'parentId');
    ...
    async hooks(ctx, hooks, params = {}) {
        try {
            ...
            let targetHook = await ctx.service.hook.item({
                query: {
                    name: hooks
                },
                attributes: ['id']
