...
var searchPannelVM = avalon.define({
    $id: 'searchPannel',
    lsk: [],
    searchkey: '',
    searchOpt: function (e) {
        if (searchPannelVM.searchkey) {
            window.location.href = '/search/' + searchPannelVM.searchkey;
        } else {
            layer.msg(getSysValueByKey('sys_layer_validate_keyword'), {
                icon: 0,
                time: msgTime
...
function getUserContentsInfo(userId) {
    getAjaxData('/api/content/getList?userId=' + userId + '&pageSize=5', (data) => {
        // console.log('data', data)
        postArticelVm.authorConentsInfo = data.data;
    })
}
 ...
 * 
 */
var phoneListVm = avalon.define({
    $id: 'phonelist',
    myContentList: [],
    contentTotalPage: 1,
    contentPageClick: function (e, cur) {
        getAjaxData('/api/content/getList?current=' + cur, (data) => {
            if (data.status == 200) {
                phoneListVm.myContentList = data.data.docs;
                phoneListVm.contentTotalPage = data.data.pageInfo.totalPage;
            }
        })
    },
